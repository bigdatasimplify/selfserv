...
def get_src_df(source_cnx_info, ctl_params, spark):
...
def get_write_df(src_df, ctl_params, spark):  
    ...
    if target_hive!="":
        ...
        hive_drop_table=target_hive.get("hive_drop_table","N")        
        if hive_create_table=="Y":
            ...
            for struct in write_schema:
                v_col_nm=str(struct.name)
                v_col_type=str(struct.dataType)
                ...
                "ShortType":"Integer",
                "UnknownType":"String"}                
                v_col_type=col_type_map.get(v_col_type,"string")                
            ...
            hive_schema=f_nvl2(hive_schema,"abi_repo",hive_schema)
            if hive_schema not in hive_db_list:
                ignore_df=hc.sql("create database " + hive_schema)
            
            if f_nvl2(hive_drop_table,"N",hive_drop_table)=="Y":
               v_drop="drop table if exists " + hive_schema + "." + hive_table
               print("dropping table")
               ignore_df=hc.sql(v_drop)
            ...
            if hive_table_type=="managed":            
                v_ddl="create table IF NOT EXISTS " + hive_schema + "." + hive_table + " (" + v_col_str + ") " + f_nvl2(tgt_partition_columns,"", " partitioned by (" + v_part_col_str + ") ") + v_txt_params + " STORED AS " + v_hdfs_format 
                print(v_ddl)
                hc.sql(v_ddl)
                src_df.registerTempTable("src_df");
                
                insert_sql="insert into " + hive_schema + "." + hive_table + f_nvl2(tgt_partition_columns,""," partition (" + tgt_partition_columns + ") ") + " select " + v_col_only_str + f_nvl2(tgt_partition_columns,"",", " + tgt_partition_columns ) + " from  src_df "  
                print(insert_sql) 
                hc.sql(insert_sql)
                
            else:
                v_hive_table_type="external"
                v_ddl="create " + v_hive_table_type + " table IF NOT EXISTS " +  hive_schema + "." + hive_table + " (" + v_col_str + ") " + f_nvl2(tgt_partition_columns,"","partitioned by (" + v_part_col_str + ") ")+v_txt_params+" STORED AS " + v_hdfs_format + " location '" +hdfs_tgt_dir+"' "                 
                print(v_ddl)    
                hc.sql(v_ddl)
...
sc = SparkContext(appName = "SparkIngestApp")
...
src_df=get_src_df(source_cnx_info, ctl_params, spark)
write_df=get_write_df(src_df,ctl_params, spark)
